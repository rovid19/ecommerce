...
const PostModal = ({ setPostModalVisible, feedPosts, index }) => {
  const user = useSelector((state) => state.userData.value.user);
  console.log(user);
  return (
    <div className="absolute top-0 h-full w-full bg-neutral-900 bg-opacity-60 z-50 flex justify-center items-center">
      <article className="h-[100%] w-[70%] bg-neutral-900  relative rounded-md p-4">
        <button
        ...
        <div className="h-[10%] w-full flex">
          ...
          <div className="w-[80%] h-full p-4 text-neutral-400">
            <h1 className="text-xl">{feedPosts[index].postAuthor.username}</h1>
            <h3 className="text-sm">{feedPosts[index].postDate}</h3>
          </div>
        </div>
        <div className="h-[50%] w-ful relative">
          {" "}
        ...
        <div className="h-[30%] w-full">
          ...
          <div className="h-[40%] flex p-2 gap-2 relative">
            <img
              className="h-[50%] w-[5%] rounded-full object-cover"
              src={user.profilePicture}
            ></img>
            <textarea className="h-full w-[95%] align-top p-2 bg-neutral-800 text-white outline-none" />
